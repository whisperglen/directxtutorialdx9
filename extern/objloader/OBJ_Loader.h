...
namespace objl
{
	...
	namespace algorithm
	{
			...
			std::string token)
		{
			...
			const char *incstr = in.c_str();
			const char *tokencstr = token.c_str();

			int insz = in.size();
			int tokensz = token.size();

			for (int i = 0; i < insz; i++)
			{
				//std::string test = in.substr(i, token.size());
				const char *test = incstr + i;

				if (/*test == token*/ 0 == strncmp(test, tokencstr, tokensz))
				{
					if (!temp.empty())
					{
						out.push_back(temp);
				...
				else if (i + tokensz >= insz)
				{
					temp += in.substr(i, tokensz);
					out.push_back(temp);
					break;
				}
				else
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "mtllib")
				{
					...
					const char *sep = "/";
					if (Path.find(sep, 0) == Path.npos && Path.find("\\", 0) != Path.npos)
						sep = "\\";
					std::vector<std::string> temp;
					algorithm::split(Path, temp, sep);
