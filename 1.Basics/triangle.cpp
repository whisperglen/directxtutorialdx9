...
public:
	...
	{
		// create three vertices using the CUSTOMVERTEX struct built earlier
		fvf_2dvertcoltex vertices[] =
		{
			{ 10, 310, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 0, 0), },
			{ 10, 10, 0.5f, 1.0f, D3DCOLOR_XRGB(255, 0, 0), },
			{ 420, 10, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 0, 255), },
			{ 420, 310, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 255, 0), },
		};
	...
	virtual int loop(LPDIRECT3D9 d3d, LPDIRECT3DDEVICE9 device)
	{
		...
		// select which vertex format we are using
		device->SetFVF(FVF_2DVERTCOLTEX);

		// select the vertex buffer to display
		device->SetStreamSource(0, v_buffer, 0, sizeof(fvf_2dvertcoltex));
